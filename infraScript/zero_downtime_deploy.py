...
class ServiceManager:
    # 초기화 함수
    def __init__(self, socat_port: int = 8105, sleep_duration: int = 3) -> None:
        self.socat_port: int = socat_port
        self.sleep_duration: int = sleep_duration
        self.services: Dict[str, int] = {
            'here_is_paw_careCenter_1': 8106,
            'here_is_paw_careCenter_2': 8107
        }
    ...
    def _find_current_service(self) -> None:
        ...
        if not current_service:
            self.current_name, self.current_port = 'here_is_paw_careCenter_2', self.services['here_is_paw_careCenter_2']
        else:
    ...
    def _cleanup_dangling_images(self) -> None:
        ...
        # 먼저 이미지 목록 확인
        cmd_list = "docker images | grep '.*/here-is-paw/here_is_paw_careCenter' | grep '<none>'"
        ...
        # 이미지 정리
        cmd_remove = "docker images | grep '.*/here-is-paw/here_is_paw_careCenter' | grep '<none>' | awk '{print $3}' | xargs -r docker rmi"
    ...
    def _run_container(self, name: str, port: int) -> None:
        os.system(
            f"docker run -d --name={name} --restart unless-stopped -p {port}:8105 -e TZ=Asia/Seoul -v /dockerProjects/here_is_paw_msa/careCenter/volumes/gen:/gen --pull always ghcr.io/here-is-paw/here_is_paw_careCenter")
##
